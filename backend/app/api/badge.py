...
async def get_badges(token: str = Depends(oauth2_scheme), db: Session = Depends(get_db)):
    """Return all badge rows linked to the currently authenticated user."""
    user_id = get_user_id_from_token(token)
    user_badges = badge_crud.get_user_badges(db, user_id)
